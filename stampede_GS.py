...
def genSubScript(reRelax,jName,dirList,runLength,nCores,nNodes):
...
def getLat(jName, aList, reRelax, runLength,nCores,nNodes):
    ...
    for a in aList:
        ...
        sp.call('cp POSCAR INCAR_static INCAR_relax KPOINTS POTCAR'.split()+\
                [dirName])
        if reRelax:
            sp.call(['cp','INCAR_cont',dirName])
        sp.call('cp -r %s %s'%(dirName,WORK),shell=True)
    # create submission script and run
    genSubScript(reRelax,jName,dirList,runLength,nCores,nNodes)
...
print 'a values:'
print aList,'\n'
getLat(jName,aList,reRelax,runLength,nCores,nNodes)
